...
CREATE OR REPLACE PACKAGE BODY pkg_manejo_logs AS
    ...
    -- Procedimiento para insertar un log en un archivo plano CSV
    PROCEDURE pr_insertar_log_archivo(
        p_evento IN VARCHAR2,
        p_momento IN VARCHAR2,
        p_accion IN VARCHAR2,
    ...
    BEGIN
        -- Verifica si los logs y los logs de archivo están activados
        IF log_activo AND log_archivo_activo THEN
            -- Obtener el usuario actual y la fecha
            usuario_actual := SYS_CONTEXT('USERENV', 'SESSION_USER');
            fecha_actual := SYSTIMESTAMP;

            -- Validación de cabecera y manejo de edición incorrecta
            BEGIN
            ...
            UTL_FILE.PUT_LINE(v_archivo, TO_CHAR(fecha_actual, 'YYYY-MM-DD HH24:MI:SS') || ',' ||
                                           usuario_actual || ',' ||
                                           p_tabla || ',' ||
                                           p_evento || ',' ||
                                           p_momento || ',' ||
    ...
    EXCEPTION
        WHEN OTHERS THEN
            ...
            -- Registrar el error en archivo separado
            DECLARE
